...
export function KineticFieldPage() {
  ...
  const sceneMotion = disconnected ? { ...motion, horizontal: 0, vertical: 0, energy: 0, isStill: true } : motion

  useEffect(() => {
    if (Math.abs(motion.horizontal) > .05 || Math.abs(motion.vertical) > .05 || motion.energy > .04) setInteracted(true)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setStableField(true), STABLE_DELAY_MS)
    return () => clearTimeout(timer)
  }, [disconnected, motion.isStill, motion.energy, motion.horizontal, motion.vertical])

  const stateLabel = useMemo(() => {
    if (disconnected) return 'CALMING'
    if (stableField) return 'STABLE FIELD'
    if (motion.energy > BURST_THRESHOLD || !motion.isStill) return 'FIELD ACTIVE'
    return 'SETTLING'
  }, [disconnected, motion.energy, motion.isStill, stableField])

  const subtitle = 'Move your hand. Control the field.'

  return <AppShell><main className="kinetic-page">
    <section className="kinetic-heading">
      <div>
        <h1>Kinetic Field</h1>
        <p>{subtitle}</p>
      </div>
      <div className="kinetic-header-actions"><div className={`kinetic-live-status ${source === 'oura' ? 'live' : ''}`}><strong><i />{source === 'oura' ? 'Live mode' : 'Demo mode'}</strong><span>{source === 'oura' ? 'Using Oura Ring' : 'Mouse and keyboard'}</span></div><button className="kinetic-fullscreen" onClick={() => document.documentElement.requestFullscreen?.()} aria-label="Enter fullscreen">⛶</button><details className="kinetic-connection"><summary>Connection</summary><div><Link to="/" className="back">← Home</Link><ConnectionStatus /></div></details></div>
    </section>
    ...
    <section className="kinetic-stage">
      ...
      <div className="kinetic-grid">
        <div className="kinetic-main-column"><div className="kinetic-scene-shell">
          <KineticFieldScene motion={sceneMotion} mode={mode} formation={formation} radius={radius} quality={quality} disconnected={disconnected} resetSignal={resetSignal} />
          <DirectionArrow horizontal={motion.horizontal} vertical={motion.vertical} energy={motion.energy} />
          {disconnected && <div className="kinetic-warning" role="status"><strong>Live connection needs attention</strong><p>The field will stay rendered while motion settles back into formation.</p><button onClick={useDemoMode}>Return to Demo Mode</button></div>}
        </div>
