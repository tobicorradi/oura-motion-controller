...
import { BURST_DECAY, FIELD_HEIGHT, FIELD_MAX_RADIUS, FIELD_WIDTH } from './constants'
import { computeForceCenter, computeForceFalloff, createFormationTargets, detectEnergySpike, particleCountForQuality, sanitizeMotion } from './particleMath'
...
const particlePalette = [
  new THREE.Color('#ff2f9f'), new THREE.Color('#db35ff'), new THREE.Color('#7b3cff'), new THREE.Color('#2f80ff'), new THREE.Color('#14cbd3'), new THREE.Color('#ff2f9f'),
]

function setAngularColors(colors: Float32Array, targets: Float32Array, count: number) {
  const mixed = new THREE.Color()
  for (let index = 0; index < count; index += 1) {
    const cursor = index * 3
    const angle = (Math.atan2(targets[cursor + 1], targets[cursor]) + Math.PI * 2 + Math.PI * .08) % (Math.PI * 2)
    const progress = angle / (Math.PI * 2) * 5
    const base = Math.floor(progress)
    mixed.copy(particlePalette[base]).lerp(particlePalette[base + 1], progress - base)
...
export function ParticleField({ motion, mode, formation, radius, quality, disconnected, reducedMotion, resetSignal }: Props) {
  ...
  useEffect(() => {
    ...
    const rest = createFormationTargets(formationRef.current, count, radiusRef.current)
    for (let index = 0; index < count; index += 1) {
      const cursor = index * 3
      ...
      positions[cursor + 2] = rest[cursor + 2] + Math.sin(seed * 64) * jitter * 0.75
      velocities[cursor] = 0
      velocities[cursor + 1] = 0
      velocities[cursor + 2] = 0
      scales[index] = 0.58 + (seed * .9)
      seeds[index] = seed
    }
  ...
  useFrame((state, delta) => {
    ...
    const current = sanitizeMotion(disconnected ? { horizontal: 0, vertical: 0, energy: 0, isStill: true } : motion)
    const live = smoothedMotion.current
    live.horizontal = THREE.MathUtils.damp(live.horizontal, current.horizontal, 6.4, safeDelta)
    live.vertical = THREE.MathUtils.damp(live.vertical, current.vertical, 6.4, safeDelta)
    live.energy = THREE.MathUtils.damp(live.energy, current.energy, current.isStill ? 3.1 : 5.8, safeDelta)

    if (current.isStill) stableTimer.current += safeDelta
    else stableTimer.current = 0

    const now = performance.now()
    if (detectEnergySpike(live.energy, previousEnergy.current, now, lastBurstAt.current)) {
    ...
    burstStrength.current = Math.max(0, burstStrength.current - safeDelta * BURST_DECAY)
    burstProgress.current += safeDelta * (2.4 + live.energy * 1.8)

    const directionalForce = 0.26 + live.energy * 1.04
    const restStrength = stableTimer.current > .9 ? 0.082 : current.isStill ? 0.056 : 0.016
    const center = computeForceCenter(live.horizontal, live.vertical, FIELD_WIDTH * .58, FIELD_HEIGHT * .62)
    const buffersNow = buffers.current
    const turbulence = reducedMotion ? 0.01 + live.energy * 0.035 : 0.02 + live.energy * 0.085
    ...
    tempDirection.set(live.horizontal, -live.vertical, live.horizontal * 0.16).multiplyScalar(directionalForce)
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const distance = Math.max(.0001, tempCenterVector.length())
      const falloff = computeForceFalloff(distance, mode === 'flow' ? 3.5 : 3.15)
      ...
      if (mode === 'repel') {
        ...
        vz += (tempDirection.z + (tempCenterVector.x - tempCenterVector.y) * 0.12) * falloff * 1.2
      }
      ...
      vz += Math.sin(time * 0.54 + seed * 41 + (px + py) * 0.4) * turbulence * 0.7

      if (burstStrength.current > 0) {
        const pulse = 1 - Math.min(1, Math.abs(distance - burstRadius) / burstWidth)
        if (pulse > 0) {
          const push = pulse * burstStrength.current * (reducedMotion ? 0.12 : 0.22)
    ...
    uniforms.uGlow.value = reducedMotion ? 0.3 : 0.58 + live.energy * 0.32 + burstStrength.current * 0.22

    group.current.rotation.y = THREE.MathUtils.damp(group.current.rotation.y, live.horizontal * 0.07, 4.2, safeDelta)
    group.current.rotation.x = THREE.MathUtils.damp(group.current.rotation.x, live.vertical * 0.05, 4.2, safeDelta)
    group.current.position.x = THREE.MathUtils.damp(group.current.position.x, live.horizontal * 0.12, 4.8, safeDelta)
    group.current.position.y = THREE.MathUtils.damp(group.current.position.y, -live.vertical * 0.08, 4.8, safeDelta)
