...
function CameraRig({
  ...
  useFrame((_, delta) => {
    ...
    focus.set(
      motion.horizontal * 0.26,
      -motion.vertical * 0.18,
      10 - motion.energy * (reducedMotion ? 0.12 : 0.28),
    );
...
function SceneFallback({ message }: { message: string }) {
  return (
    <div className="kinetic-fallback" role="status">
      <strong>Kinetic Field unavailable</strong>
      <p>{message}</p>
      <span>
        Keyboard and demo controls remain available in the rest of the
        experience.
      </span>
    </div>
  );
...
export function KineticFieldScene(props: Props) {
  ...
  if (!webGlReady)
    return (
      <SceneFallback message="WebGL is not available in this browser, so the particle field cannot be rendered here." />
    );

  return (
    <SceneErrorBoundary
      fallback={
        <SceneFallback message="The particle scene could not initialize cleanly. Switch to Demo Mode or refresh to try again." />
      }
    >
      <Canvas
        dpr={
