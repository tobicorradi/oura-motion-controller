...
export function sanitizeMotion(
  input: Partial<FieldMotion> | undefined,
): FieldMotion {
  return {
...
export function computeForceCenter(
  horizontal: number,
  vertical: number,
  width: number,
  height: number,
...
export function particleCountForQuality(
  quality: FieldQuality,
  reducedMotion = false,
) {
  const counts: Record<FieldQuality, number> = {
    high: 7600,
    balanced: 6000,
    low: 3400,
  };
...
export function createWaveTargets(
  count: number,
  width = 3.2,
  depth = 3.2,
  amplitude = 0.28,
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    targets[cursor + 1] =
      Math.sin(nx * Math.PI * 1.3) * amplitude +
      Math.cos(ny * Math.PI * 1.6) * amplitude * 0.55;
...
export function createFormationTargets(
  ...
  if (formation === "wave")
    return createWaveTargets(
      count,
      radius * 1.45,
      radius * 1.45,
      radius * 0.12,
    );
  return createRingTargets(count, radius, Math.min(0.9, radius * 0.39));
}
