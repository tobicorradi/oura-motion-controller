...
import { clampInput, computeForceCenter, computeForceFalloff, createFormationTargets, createRingTargets, createSphereTargets, createWaveTargets, detectEnergySpike, particleCountForQuality, sanitizeMotion } from './particleMath'

describe('kinetic field particle math', () => {
  it('maps motion into a centered force point', () => {
    expect(computeForceCenter(.5, -.25, 4, 2)).toEqual({ x: 2, y: .5, z: 0 })
  })

  it('clamps and sanitizes invalid input values', () => {
    expect(clampInput(4)).toBe(1)
    expect(clampInput(Number.NaN)).toBe(0)
    expect(sanitizeMotion({ horizontal: Infinity, vertical: -.4, energy: -3, isStill: 0 as never })).toEqual({ horizontal: 0, vertical: -.4, energy: 0, isStill: false })
  })

  it('detects energy spikes with a cooldown', () => {
    expect(detectEnergySpike(.42, .12, 1000, 0, .2, 600)).toBe(true)
    expect(detectEnergySpike(.42, .12, 1200, 900, .2, 600)).toBe(false)
  })

  it('computes a smooth force falloff by distance', () => {
    expect(computeForceFalloff(0, 3)).toBe(1)
    expect(computeForceFalloff(1.5, 3)).toBeGreaterThan(.45)
  ...
  it('creates ring formation targets', () => {
    ...
    expect(Math.abs(targets[2])).toBeLessThan(.6)
  })
  ...
  it('creates wave formation targets', () => {
    ...
    expect(Math.abs(targets[1])).toBeLessThan(.5)
  ...
  it('maps quality levels to particle counts', () => {
    expect(particleCountForQuality('high')).toBeGreaterThan(particleCountForQuality('balanced'))
    expect(particleCountForQuality('low')).toBeLessThan(particleCountForQuality('balanced'))
    expect(particleCountForQuality('balanced', true)).toBeLessThan(particleCountForQuality('balanced'))
  })
})
