...
export class MotionSynthEngine {
  ...
  async enable() {
    ...
    this.master?.gain.setTargetAtTime(.07, this.context.currentTime, .28)
  }
  ...
  private createGraph() {
    ...
    this.filter.type = 'lowpass'; this.filter.Q.value = .55; this.master.gain.value = 0; this.voiceGain.gain.value = .018; this.analyser.fftSize = 128; this.analyser.smoothingTimeConstant = .9
  ...
  private applyPreset() {
    ...
    this.delay?.delayTime.setTargetAtTime(preset.delayTime, now, .2); this.feedback?.gain.setTargetAtTime(Math.min(.18, preset.feedback), now, .2)
    this.lfo?.frequency.setTargetAtTime(Math.min(.7, preset.lfoRate), now, .2); this.lfoGain?.gain.setTargetAtTime(preset.lfoDepth * .012, now, .2)
  }
  ...
  update({ frequency, filterFrequency, energy, isStill }: SynthParameters) {
    ...
    const safeFrequency = clampSynth(frequency, 80, 520); const safeFilter = clampSynth(filterFrequency, 260, 2400)
    this.oscillatorA?.frequency.setTargetAtTime(safeFrequency, now, preset.smoothing)
    this.oscillatorB?.frequency.setTargetAtTime(safeFrequency * (this.currentPreset === 'cosmic' ? 1.498 : 1.5), now, Math.max(.16, preset.smoothing))
    this.filter?.frequency.setTargetAtTime(safeFilter, now, .09)
    this.voiceGain?.gain.setTargetAtTime(clampSynth(preset.baseGain * .46 * calm + intensity * .012, .007, .042), now, .22)
    this.lfoGain?.gain.setTargetAtTime(preset.lfoDepth * (isStill ? .003 : .008 + intensity * .012), now, .24)
  }
  triggerPulse() {
    if (!this.context || !this.voiceGain || !this.filter) return
    const now = this.context.currentTime; const gain = this.voiceGain.gain.value
    this.voiceGain.gain.cancelScheduledValues(now); this.voiceGain.gain.setTargetAtTime(Math.min(.05, gain + .011), now, .07); this.voiceGain.gain.setTargetAtTime(gain, now + .2, .22)
    this.filter.frequency.setTargetAtTime(Math.min(2400, this.filter.frequency.value * 1.18), now, .08)
    if (this.pulseTimer) clearTimeout(this.pulseTimer)
    this.pulseTimer = window.setTimeout(() => this.filter?.frequency.setTargetAtTime(Math.max(300, this.filter!.frequency.value * .82), this.context?.currentTime ?? 0, .25), 240)
  }
