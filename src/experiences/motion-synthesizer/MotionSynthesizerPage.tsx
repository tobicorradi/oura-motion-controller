...
export function MotionSynthesizerPage() {
  ...
  return (
    <AppShell accent="cyan">
      <main className="synth-page wave-page">
        ...
        <section className="synth-heading">
          <div>
            <p className="eyebrow">Experience 03 / Motion waves</p>
            <h1>
              Motion
              <br />
              <i>Waves</i>
            ...
            <p>
              Shape a visual current with the smallest movement. A silent study
              in gesture, energy and flow.
            </p>
          </div>
          <div className="wave-status">
            ...
            <strong>Silent field</strong>
            <small>Move the ring to bend the field.</small>
          </div>
        </section>
        <section className="synth-stage wave-stage">
          ...
          <div
            ...
            role="group"
            aria-label="Visual wave preset"
          >
            {(Object.keys(presets) as SynthPreset[]).map((key) => (
              <button
                key={key}
              ...
              >
                {presets[key].label}
                <kbd>
                  {key === "ambient" ? "1" : key === "pulse" ? "2" : "3"}
                </kbd>
              </button>
            ))}
          </div>
          <p className="visual-description">
            Horizontal movement changes the current. Vertical movement bends the
            wave field.
          </p>
        </section>
        <section className="synth-hud" aria-label="Motion wave values">
          ...
          <div>
            ...
            <strong>{signed(motion.normalizedRoll)}</strong>
          </div>
          <div>
            <span>Vertical</span>
            <strong>{signed(motion.normalizedPitch)}</strong>
          </div>
          <div>
            <span>Energy</span>
            <strong>{Math.round(motion.energy * 100)}%</strong>
          </div>
          <div>
            <span>Source</span>
        ...
        <footer className="synth-footer">
          <p>
            Move left / right to shift the current · Move up / down to bend the
            field · <kbd>Space</kbd> to send a visual pulse
          </p>
          <div>
