...
describe("spatial environment utilities", () => {
  ...
  it("uses a neutral dead zone for invalid and tiny motion", () => {
    expect(applyWheelDeadZone(0.08)).toBe(0);
    expect(applyWheelDeadZone(Number.NaN)).toBe(0);
    expect(applyWheelDeadZone(-0.4)).toBe(-0.4);
  });

  it("switches categories once per threshold crossing and cooldown", () => {
    expect(shouldSwitchCategory(0.6, true, 1000, 0)).toBe(true);
    expect(shouldSwitchCategory(0.6, true, 1200, 1000)).toBe(false);
    expect(releaseCategorySwitch(0.2)).toBe(true);
