...
export function SpatialEnvironmentPage() {
  ...
  const atmosphere = useMemo(() => createAtmosphere(temperature, 'Soft Neutral', 'Gentle'), [temperature])

  return <AppShell accent="gallery"><main className="environment-page" style={atmosphereStyle(atmosphere)}>
    <section className="environment-heading"><div><p className="eyebrow">Experience 05 / Temperature wheel</p><h1>Spatial Environment</h1><p>Shape warmth through motion.</p></div><div className="environment-actions"><button onClick={wheel.toggleAutoDemo} aria-pressed={wheel.autoDemo}>Auto Demo {wheel.autoDemo ? 'On' : 'Off'}</button><button onClick={wheel.reset}>Reset <kbd>R</kbd></button><details><summary>Connection</summary><div><Link to="/" className="back">← Home</Link><ConnectionStatus /></div></details></div></section>
    <div className="environment-single-category" aria-label="Active category"><i>♨</i>Temperature</div>
    <section ref={viewportRef} className="environment-stage" aria-label="Motion controlled temperature wheel. Move vertically to adjust the temperature."><div className="environment-ambient" /><EnvironmentWheel category="temperature" selection={wheel.selection} position={wheel.wheelPosition} positionRef={wheel.wheelPositionRef} /><EnvironmentPreview temperature={temperature} atmosphere={atmosphere} />{disconnected && <div className="environment-warning"><strong>Live connection needs attention</strong><button onClick={useDemoMode}>Return to Demo Mode</button></div>}</section>
    <section className="environment-instructions"><i>✦</i><div><strong>Control the temperature</strong><p>Move up or down to adjust the temperature.<br />Hold still to let the wheel settle.</p></div></section>
  </main></AppShell>
}
